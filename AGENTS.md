...
components/
  layouts/
    dashboard.py    # AppShell structure — imports nav_links, nav_bar, sidebar
  ui/
    nav_links.py    # NAV_ITEMS config list + _build_link builder — edit here to add nav entries
    metric_card.py  # Reusable KPI card widget
    settings_popover.py
    theme_toggle.py
callbacks/
  register.py       # Central callback registration — import and call register_*_callbacks(app) here
  theme.py          # Theme/color-scheme + sidebar toggle callbacks
styles/
  flowtides_theme.py  # Single source of truth for theme, colors, layout helpers, Plotly templates
api/
  __init__.py
  routes/
    default.py      # FastAPI route handlers added to app.server
data/
  models.py         # Pydantic v2 models for Well, Rig, and all sub-entities
  _tempales/        # Well template JSON files
  benedum_pad/      # Pad-level data
```
...
3. Add an entry to `NAV_ITEMS` in `components/ui/nav_links.py` — no changes to `dashboard.py` needed.

```python
# Top-level link
{"label": "Reports", "icon": "tabler:report", "href": "/reports"},

# Sub-item under an existing accordion parent (e.g. Tools)
{"label": "Torque & Drag", "icon": "tabler:rotate-clockwise", "href": "/tools/torque-drag"},

# Accordion parent (default open)
{"label": "Tools", "icon": "tabler:tools", "href": None, "opened": True, "children": [...]}
```
...
### Theme
- Primary color is `blue`. Use `color="blue"` on all DMC components.
- The full theme dict lives in `styles/flowtides_theme.py` — edit component `defaultProps` there rather than setting props on every instance.
...
Pydantic v2 models. Load a well from JSON with `Well.model_validate(raw_dict)`.

### Root model

```python
class Well(BaseModel):
    header: WellHeader       # operator info, location, rig, status, contacts
    wellbores: list[Wellbore]
    csg_liners: list[CsgLiner]
...
| Model | Key fields |
|-------|-----------|
| `WellHeader` | well_name, api_number, afe_number, operator, status, well_type, spud_date, location, operator_contact, **rig**, users, notes |
| `Rig` | rig_number, rig_name, contractor, rig_type, status, max_depth_ft, hp, contact |
| `RigContact` | name, phone, email |
| `Location` | country, state, county, latitude, longitude |
| `OperatorContact` | name, email, phone |
| `WellboreSection` | name, size (in), depth_md (ft), status |
| `CsgLinerSection` | name, size_od_in, size_id_in, top_depth_md, btm_depth_md, status |
| `DrillStringComponent` | type, od, size_od, size_id, length, wellbore_ref |
| `MudSection` | name, type (WBM/OBM/SBM), depth range, mw_min/max_ppg, status |
| `SurveyStation` | md, inc, azi, tvd |
| `Formation` | name, top_md, btm_md, top_tvd, btm_tvd |

### Enums

`WellStatus` · `WellType` · `SectionStatus` · `DrillStringStatus` · `MudType` · `MudStatus` · `DirectionalStatus` · `RigType` · `RigStatus`

### Well template file

`data/_tempales/4S_well_template.json` — reference JSON matching the full `Well` schema.
...
- `primaryColor = "blue"` — use `color="blue"` on all DMC components.
...
| Size | Context |
|------|---------|
| 16 px | Inline, nav sub-items |
| 20 px | Nav top-level items |
| 24 px | Card headers |
| 32 px | Feature / hero |
