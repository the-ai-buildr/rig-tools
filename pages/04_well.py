...
from utils.project_store import get_project, get_well, update_well
...
# ---------------------------------------------------------------------------
# Guard — must have active project + well
# ---------------------------------------------------------------------------
...
if well is None:
    st.warning("No well selected. Please open a well from the Project page.")
    if st.button(":material/arrow_back: Back to Project"):
...
with st.sidebar:
    ...
    horizontal_rule()
    if st.button(":material/arrow_back: Back to Project", use_container_width=True):
        st.switch_page("pages/03_project.py")
...
with tab_header:
    ...
    with st.form("form_well_header"):
        ...
        with c1:
            ...
            well_name   = st.text_input("Well Name",    value=h.well_name)
...
# ===========================================================================
with tab_wellbores:
    st.markdown("#### Wellbores")

    wb_data = [
        {
            "Name": wb.name,
            "Type": wb.wellbore_type,
            "MD (ft)": wb.measured_depth,
            "TVD (ft)": wb.true_vertical_depth,
        }
        for wb in well.wellbores
    ]
    df_wb = pd.DataFrame(wb_data) if wb_data else pd.DataFrame(
        columns=["Name", "Type", "MD (ft)", "TVD (ft)"]
    )
    ...
    edited_wb = st.data_editor(
        ...
        column_config={
            "Type": st.column_config.SelectboxColumn(
                "Type",
                options=["Vertical", "Horizontal", "Deviated"],
            ),
    ...
    if st.button(":material/save: Save Wellbores", type="primary", key="save_wb"):
        well.wellbores = [
            Wellbore(
                wellbore_id=_new_id(),
...
with tab_casings:
    ...
    csg_data = [
        {
            ...
            "OD (in)": c.od,
            "ID (in)": c.id_,
            "Weight (lb/ft)": c.weight,
            "Grade": c.grade,
            "Top (ft)": c.top_depth,
            "Bottom (ft)": c.bottom_depth,
            "Cement Top (ft)": c.cement_top,
        }
        for c in well.casings
    ]
    df_csg = pd.DataFrame(csg_data) if csg_data else pd.DataFrame(
        columns=["Name", "Type", "OD (in)", "ID (in)", "Weight (lb/ft)",
                 "Grade", "Top (ft)", "Bottom (ft)", "Cement Top (ft)"]
    )

    edited_csg = st.data_editor(
        df_csg,
        num_rows="dynamic",
        use_container_width=True,
        column_config={
            "Type": st.column_config.SelectboxColumn(
                "Type", options=["Casing", "Liner"]
            ),
    ...
    if st.button(":material/save: Save Casings & Liners", type="primary", key="save_csg"):
        well.casings = [
            CasingLiner(
                id=_new_id(),
                name=str(row.get("Name", "")),
            ...
            for _, row in edited_csg.iterrows()
            if str(row.get("Name", "")).strip()
        ]
        update_well(project_id, well)
...
with tab_mud:
    ...
    mud_data = [
        {
            ...
            "Mud Type": m.mud_type,
            "MW (ppg)": m.mud_weight,
            "Viscosity (cP)": m.viscosity,
            "pH": m.ph,
            "Chlorides (mg/L)": m.chlorides,
        }
        for m in well.mud_entries
    ]
    df_mud = pd.DataFrame(mud_data) if mud_data else pd.DataFrame(
        columns=["Date", "Depth (ft)", "Mud Type", "MW (ppg)", "Viscosity (cP)", "pH", "Chlorides (mg/L)"]
    )

    edited_mud = st.data_editor(
        df_mud,
        num_rows="dynamic",
        use_container_width=True,
        column_config={
    ...
    if st.button(":material/save: Save Mud Table", type="primary", key="save_mud"):
        well.mud_entries = [
            MudEntry(
                id=_new_id(),
            ...
            for _, row in edited_mud.iterrows()
